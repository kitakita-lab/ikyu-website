...
export const PETAL_CONFIG = {
  /**
   * 同時に表示する上限。
   * スマホは横流れで画面外へ抜ける枚数がある分、実測の見え方はPCより少なくなる。
   * 上限7では平均がPCを下回ったため、PCと同じ8に揃えた。
   * 常時この枚数が出るわけではなく、実測の平均はPCで約5.7枚・スマホで約5.5枚
   */
  maxConcurrent: { desktop: 8, mobile: 8 },
  /** 次の1枚が現れるまでの間隔(秒)。この範囲でランダム(以前は 2.6〜5.4) */
  gapSec: { min: 1.8, max: 3.8 },
  /**
   * ときどき入れる「花びらのない間」(秒)と、その確率(以前は 8〜12 秒・12%)。
   * 間隔のゆらぎとしては残すが、途切れたと感じる長さにはしない
   */
  restSec: { min: 7, max: 10 },
  restChance: 0.08,
  /** 初回表示:最初の1枚が現れるまでの秒数(ページを開いた直後だけ) */
  firstDelaySec: 0.15,
  /**
   * 初回表示:数枚目までを詰める間隔(秒)。
   * 開いて5〜8秒で「花びらが舞っている」と分かる密度にするため(以前は 1.2〜2.4)。
   * 一斉には出さず、あくまで時間差で1枚ずつ足していく
   */
  openingGapSec: { min: 0.9, max: 1.9 },
  /** 上の詰めた間隔を使う枚数(これ以降は通常の間隔に戻る。以前は 3) */
  openingCount: 5,
  /**
   ...
   * PCは1枚の滞在時間が短く枚数が溜まりにくいため、ヒーロー内に収まる範囲で
   * 距離を伸ばした(以前は 40〜56)。1枚ごとの寿命の差も広がる
   */
  fallVh: { desktop: [46, 64], mobile: [56, 74] },
  /**
